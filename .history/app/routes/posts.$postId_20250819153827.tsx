...
export async function action({ request, params }: ActionFunctionArgs) {
  ...
  if (intent === "edit_comment") {
    ...
    // Verify the user can edit this comment
    const isAuthorizedToEdit = authorEmail.trim().toLowerCase() === currentComment.author_email?.toLowerCase();
    if (!isAuthorizedToEdit) {
      return Response.json(
        { error: "You can only edit your own comments. Please use the same email address you used when creating the comment." },
        { status: 403 }
      );
    }

    // Prevent guests from editing comments created by registered users (and vice versa)
    const isCurrentCommentByGuest = isGuestEmail(currentComment.author_email || '');
    const isEditRequestByGuest = isGuestEmail(authorEmail.trim());
    
    if (isCurrentCommentByGuest !== isEditRequestByGuest) {
      return Response.json(
        { error: "Guest users can only edit guest comments, and registered users can only edit registered posts." },
        { status: 403 }
      );
    }
