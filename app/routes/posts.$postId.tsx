...
export async function action({ request, params }: ActionFunctionArgs) {
  ...
  if (intent === "comment") {
    ...
    try {
      ...
      return Response.json(
        { error: "Failed to post comment due to unexpected error" },
        { status: 500 }
      );
    }
  }
  ...
  if (intent === "edit") {
    // Require authentication for editing posts
    const user = await getOptionalUser(request);
    if (!user) {
      return Response.json(
    ...
    // Verify the user can edit this post (must be the author)
  ...
  if (intent === "edit_comment") {
    // Require authentication for editing comments
    const user = await getOptionalUser(request);
    if (!user) {
      return Response.json(
    ...
    // Fetch current comment to verify ownership
    ...
    // Verify the user can edit this comment (must be the author)
    if (currentComment.author_id !== user.id) {
      return Response.json(
    ...
    return Response.json({ success: true, comment: updatedComment });
  }
...
export default function PostDetail() {
  const { post, comments, totalComments, user, userProfile } = useLoaderData<typeof loader>();
  const actionData = useActionData<{ error?: string; success?: boolean }>();
  const navigation = useNavigation();
  const [showReplyForm, setShowReplyForm] = useState(false);
  const [isEditingPost, setIsEditingPost] = useState(false);
  // Remove unused state since we now have authenticated users
  ...
  const canEditPost = user && post.author_id === user.id;
    ...
    <div className="max-w-4xl mx-auto px-4 py-8">
      ...
      <nav className="mb-8 text-lg text-slate-600">
        ...
        {post.category && (
          <>
            <span className="mx-3 text-xl">›</span>
            <Link to={`/categories/${post.category.slug}`} className="hover:text-cyan-600 font-medium">
              {post.category.name}
            </Link>
          </>
        )}
            ...
            {user ? (
              <>
                ...
                <div className="flex items-center gap-3 mb-4">
                  ...
                  <span className="text-sm text-slate-600">
                    Commenting as <strong>{userProfile?.display_name || user.email?.split('@')[0] || 'Anonymous User'}</strong>
                  </span>
                </div>
                <Form method="post" className="space-y-4">
              ...
              <div className="text-center py-8">
                <div className="mb-4">
                  ...
                  <p className="text-slate-600">You need to be logged in to comment on posts.</p>
                </div>
                <div className="flex items-center justify-center gap-4">
                  <Link to="/login" className="inline-flex items-center px-4 py-2 bg-cyan-500 text-white rounded-lg hover:bg-cyan-600 transition-colors">
                    Sign In
                  </Link>
                  <Link to="/signup" className="inline-flex items-center px-4 py-2 border border-slate-300 text-slate-700 rounded-lg hover:bg-slate-50 transition-colors">
                    Create Account
                  </Link>
                </div>
              </div>
